...
import {useAddUserStore} from "@/stores/user/addUser.js";
...
const addUser = async (data) => {
    await addUserStore.addUser({...data.value});
    await usersStore.fetchUsers(queryFilters.query);
}

const changeUser = async (data) => {
    await changeUserStore.changeUser({
...
<template>
    <section class="flex flex-col w-full">
        <SearchAndAdd v-model:is-opened-modal="isOpenedModalAdd" @send="search" v-model:query-filters="queryFilters" name="Foydalanuvchi"/>
        <div class="bg-white rounded-md ">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 overflow-scroll">
                    <tr v-for="user in usersStore.users.all" :key="user.id" class="text-my-blue-gray">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">{{ timeAgo(user.lastSeen) }}</td>
                        <td class="px-6 py-4 flex gap-6">
                            <button @click="isOpenedModalChange = true; currentUserId = user.id" class="cursor-pointer">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="isOpenedModalDelete = true; currentUserId = user.id" class="cursor-pointer">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <ModalWindow
        @add-or-change="addUser"
        v-model:data="userData"
        :is-loading="addUserStore.isLoading"
        v-model:is-opened-modal="isOpenedModalAdd"
        title="Foydalanuvchi qo'shish"
    />
    <ModalWindow
        @add-or-change="changeUser"
        v-model:data="userData"
        :is-loading="changeUserStore.isLoading"
