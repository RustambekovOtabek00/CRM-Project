...
import {useAddCompanyStore} from "@/stores/company/addCompany.js";
import {useChangeCompanyStore} from "@/stores/company/changeCompany.js";
import {useDeleteCompanyStore} from "@/stores/company/deleteCompany.js";
import AlertNotification from "@/components/UI/AlertNotification.vue";

const companiesStore = useGetCompaniesStore();
const addCompanyStore = useAddCompanyStore();
const changeCompanyStore = useChangeCompanyStore();
const deleteCompanyStore = useDeleteCompanyStore();
...
companiesStore.fetchCompanies(queryFilters.query);

const addCompany = async (data) => {
    await addCompanyStore.addCompany({...data.value});
    await companiesStore.fetchCompanies(queryFilters.query);
}

const changeCompany = async (data) => {
    await changeCompanyStore.changeCompany({
    ...
    toastNotification.value = changeCompanyStore.isChanged;

    setTimeout(() => toastNotification.value = false, 3000);
}

const deleteCompany = async () => {
    await deleteCompanyStore.deleteCompany(currentUserId.value);
    await companiesStore.fetchCompanies(queryFilters.query);
...
<template>
    <section class="flex flex-col w-full">
        <SearchAndAdd v-model:is-opened-modal="isOpenedModalAdd" @send="search" v-model:query-filters="queryFilters" name="Kompaniya"/>
        <div class="bg-white rounded-md ">
            <table class="w-full">
                <thead>
                <tr class="text-my-gray border-b border-gray-200">
                    <th class="px-6 xl:px-16 py-3.5 text-left text-sm font-medium uppercase tracking-wider">Kompaniya nomi</th>
                ...
                <tr v-for="company in companiesStore.companies.all" :key="company.id" class="text-my-blue-gray">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ company.password }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ timeAgo(company.lastSeen) }}</td>
                    <td class="px-6 py-4 flex gap-6">
                        <button @click="isOpenedModalChange = true; currentUserId = company.id" class="cursor-pointer">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <button @click="isOpenedModalDelete = true; currentUserId = company.id" class="cursor-pointer">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <ModalWindow
        @add-or-change="addCompany"
        v-model:data="companyData"
        :is-loading="addCompanyStore.isLoading"
        v-model:is-opened-modal="isOpenedModalAdd"
        title="Kompaniya qo'shish"
    />
    <ModalWindow
        @add-or-change="changeCompany"
        v-model:data="companyData"
        :is-loading="changeCompanyStore.isLoading"
    ...
    <AlertNotification @deleteFunction="deleteCompany" v-model="isOpenedModalDelete" message="Rostan ham o’chirilsinmi ?"/>
</template>
