<script setup>
import {useDebouncedRef} from "@/stores/debouncedRef.js";
...
text.value = useDebouncedRef(text.value);
...
<template>
    <div class="flex bg-white px-3 lg:px-5 py-3 gap-3 shadow-md rounded-sm w-full">
        <label :for="name">
        ...
        <input v-model="text" class="outline-none" :id="name" type="text" :placeholder="name + ' qidirish'" />
    </div>
</template>
